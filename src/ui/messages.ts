import { EMOJIS } from '../utils/constants';

/**
 * Централизованная система сообщений UI
 * Устраняет дублирование и обеспечивает единообразие
 */

/**
 * Базовые форматтеры сообщений
 */
export class MessageFormatter {
    /**
     * Форматирует успешное сообщение
     */
    static success(message: string): string {
        return `✅ ${message}`;
    }

    /**
     * Форматирует сообщение об ошибке
     */
    static error(message: string): string {
        return `❌ ${message}`;
    }

    /**
     * Форматирует информационное сообщение
     */
    static info(message: string, emoji?: string): string {
        const icon = emoji || 'ℹ️';
        return `${icon} ${message}`;
    }

    /**
     * Форматирует заголовок
     */
    static header(title: string, emoji?: string): string {
        const icon = emoji || '📋';
        return `${icon} ${title}`;
    }

    /**
     * Форматирует список
     */
    static list(items: string[], numbered: boolean = false): string {
        return items
            .map((item, index) =>
                numbered ? `${index + 1}. ${item}` : `• ${item}`
            )
            .join('\n');
    }
}

/**
 * Текстовые константы для UI
 */
export const UI_TEXTS = {
    // Навигация
    navigation: {
        home: '🏠 На главную',
        back: '⬅️ Назад',
        cancel: '❌ Отменить',
        goingHome: 'Возвращаемся на главный экран',
        unknownCommand: 'Я не понимаю эту команду. Используйте кнопки меню.',
        useButtons: 'Используйте кнопки меню для навигации.',
    },

    // Общие действия
    common: {
        loading: '⏳ Загрузка...',
        success: '✅ Успешно!',
        error: '❌ Произошла ошибка',
        tryAgain: 'Попробуйте еще раз.',
        initialized: '✅ Инициализировано успешно',
        notImplemented: 'Эта функция будет добавлена в будущих обновлениях.',
    },

    // Кормление
    feeding: {
        buttonText: `${EMOJIS.FEED} Собачка поел`,
        success: '🍽️ Собачка вкусно поел!',
        recordedSuccess: '✅ Кормление записано успешно!\n\nЧто дальше?',
        detailsButton: '📝 Уточнить детали кормления',
        detailsPrompt:
            '📝 *Отредактируйте последнее кормление*\n\n' +
            'Если надо, поменяйте тип и количество корма:\n' +
            '• 50г сухого\n' +
            '• 60 влажного\n' +
            '• сухого 40г\n\n' +
            'Или запишите другое время кормления:\n' +
            '• 14:30\n' +
            '• 9:15\n\n' +
            'Или просто запишите хорошую мысль про нашу собачку, и она (мысль) привяжется к этому кормлению :)',
        detailsUpdated: '✅ Детали кормления обновлены!',
        detailsError: 'Ошибка: не найдено кормление для обновления',
        noRecentFeeding:
            'А вы не покормили собачку только что? Странно. А что же вы хотите тогда отредактировать? :)',
        stopped: '⏹️ Кормления на сегодня завершены',
        resumed: '▶️ Кормления возобновлены',
        stopPrompt: 'Чтобы возобновить кормления, нажмите "🍽️ Собачка поел"',
    },

    // Настройки
    settings: {
        header: '⚙️ Настройки\n\nВыберите что настроить:',
        foodHeader: '🍽️ корм',
        intervalHeader: '⏰ интервал',
        notificationsHeader: '🔔 уведомления',
        foodUpdated: 'Настройки корма обновлены!',
        intervalUpdated: 'Интервал кормления обновлен!',
        changesApplied:
            'Изменения вступят в силу после следующего кормления.',
    },

    // История
    history: {
        header: '📋 История кормлений\n\nВыберите период для просмотра:',
        todayHeader: '📅 История за сегодня',
        fullHeader: '📋 Полная история кормлений',
        noFeedings: '🔍 Кормления не найдены для выбранного периода',
        loading: '⏳ Загрузка истории...',
    },

    // Экспорт
    export: {
        header: '📤 Экспорт истории кормлений\n\nВыберите формат и период:',
        success: '✅ Экспорт завершен успешно!',
        error: '❌ Ошибка при экспорте данных',
        selectFormat: '❌ Сначала выберите формат файла.',
        csvSelected: '📋 Выбран CSV формат\n\nТеперь выберите период для экспорта:',
        htmlSelected: '🌐 Выбран HTML формат\n\nТеперь выберите период для экспорта:',
    },

    // Приветствие
    welcome:
        'Привет! 🐕\n\n' +
        'Этот бот поможет координировать кормление собаки.\n' +
        'Выберите действие:',

    // Статус
    status: {
        header: '📊 Статус кормления:\n\n',
        lastFeeding: '🍽️ Последнее кормление:',
        noFeedings: '🍽️ Кормлений еще не было\n\n',
        nextFeeding: '⏰ Следующее кормление',
        interval: '⏰ Интервал кормления',
        paused: '⏹️ Кормления приостановлены',
        statistics: '📊 Статистика:',
    },

    // Ошибки
    errors: {
        databaseNotInitialized:
            'Ошибка: база данных не инициализирована. Попробуйте перезапустить бота командой /start',
        servicesNotInitialized:
            'Ошибка: сервисы не инициализированы. Попробуйте перезапустить бота командой /start',
        timerNotInitialized:
            'Ошибка: сервис таймера не инициализирован. Попробуйте перезапустить бота командой /start',
        userNotFound: 'Ошибка: пользователь не найден',
        generic: 'Произошла ошибка. Попробуйте еще раз.',
    },
};

/**
 * Строители сообщений для сложных случаев
 */
export class MessageBuilder {
    /**
     * Создает сообщение о кормлении с деталями
     */
    static feedingNotification(params: {
        timestamp: string;
        username: string;
        amount: number;
        foodType: string;
        nextFeedingTime?: string;
        interval?: string;
    }): string {
        const { timestamp, username, amount, foodType, nextFeedingTime, interval } = params;

        const foodTypeRu = foodType === 'dry' ? 'сухого' : 'влажного';

        let message =
            `${UI_TEXTS.feeding.success}\n\n` +
            `${timestamp}\n` +
            `${username} дал ${amount}г ${foodTypeRu} корма\n`;

        if (nextFeedingTime && interval) {
            message += `\n⏰ Следующее кормление в ${nextFeedingTime} (через ${interval})`;
        }

        return message;
    }

    /**
     * Создает сообщение об остановке кормлений
     */
    static feedingStopped(initiatorUsername: string): string {
        return (
            `${UI_TEXTS.feeding.stopped}\n` +
            `Инициатор: ${initiatorUsername}\n\n` +
            `${UI_TEXTS.feeding.stopPrompt}`
        );
    }

    /**
     * Создает сообщение с текущими настройками
     */
    static currentSettings(params: {
        type: string;
        amount: string;
    }): string {
        const typeText = params.type === 'dry' ? 'Сухой' : 'Влажный';

        return (
            `Текущие настройки:\n` +
            `• Тип корма: ${typeText}\n` +
            `• Количество: ${params.amount} граммов`
        );
    }

    /**
     * Создает сообщение об обновлении настроек
     */
    static settingsUpdated(
        header: string,
        updates: string[],
        initiator?: string
    ): string {
        let message =
            `${MessageFormatter.success(header)}\n\n` +
            `Новые настройки: ${updates.join(', ')}\n\n` +
            `${UI_TEXTS.settings.changesApplied}`;

        if (initiator) {
            message += `\nИнициатор: ${initiator}`;
        }

        return message;
    }
}
