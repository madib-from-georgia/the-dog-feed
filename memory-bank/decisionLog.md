...
[2025-10-16 20:29:00] - Исправлена проблема с истекшим SSL сертификатом
- **Проблема:** SSL сертификат для домена makishvili.duckdns.org истек 15 октября 2025, что привело к ошибке "certificate verify failed" в Telegram webhook
- **Диагностика:** 
  - Обнаружено 65 накопившихся обновлений в Telegram из-за недоступности webhook
  - Конфликт конфигураций nginx (дублирующие server_name)
  - DNS проблемы при попытке обновления через webroot метод
- **Решение:**
  - Удален webhook для очистки очереди обновлений (65 → 0)
  - Устранен конфликт nginx конфигураций (удален temp-setup)
  - Обновлен SSL сертификат через standalone режим certbot
  - Перенастроен webhook с новым действительным сертификатом
- **Результат:**
  - SSL сертификат действителен до 14 января 2026 года
  - Webhook работает с TLS 1.3 и HTTP/2
